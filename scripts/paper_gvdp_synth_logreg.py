...
if dist == "normal":
    norm_bound = 5
    # hoping all generated values are within 5 std
elif dist == "uniform":
    norm_bound = 1

logreg_d = 3
intercept_tru = 0
beta_tru = np.random.normal(size=logreg_d)
...
lr_est = lambda data, f0: logreg_est_intercept(
    data, alg="newton", warm_start=True, f0=f0
)

data = gen_logreg_data(10000, logreg_d, beta_tru, dist=dist, intercept=intercept_tru)

f0 = lr_est(data, None)

est = lambda data: lr_est(data, f0)[dim_val]

################ N  LIST ###########################
n_lst = [1000, 2000, 4000, 6000]
...
dir_prefix = f"results/Final_logreg_est_gvdp_cov_{1 - alpha}_and_len/"
...
def parallel_trials(n, trial):
    ...
    data = gen_logreg_data(n, logreg_d, beta_tru, dist=dist, intercept=intercept_tru)

    tru_param = np.hstack([beta_tru, [intercept_tru]])[dim_val]
